...
import net.paperbon.lib.foldingimp.Imposition;
        ...
        for(Node child : children){
          ...
          if (child instanceof Heading){
            if (((Heading)child).getLevel() == 1){
              ...
              System.out.println(
                "chapter: " + ((Heading)child).getText().unescape());
              if (child.getNext() != null && 
                  child.getNext() instanceof TocBlock) {
                //this is a heading FOR the TOC page, so we will let the TOC
                // account for the necessary page, instead of double-counting.
                continue;
              } else {
                numChapters++;
              }
            }
          } else if (child instanceof TocBlock){
            hasTOC = true;
          }
        }

        int minPageCount = numChapters + numTitles + (hasTOC ? 1 : 0);
        Imposition imposition = new Imposition(minPageCount);
        System.out.println("wanted " + 
                           minPageCount + 
                           " pages, got " + 
                           imposition.getNumPages());
